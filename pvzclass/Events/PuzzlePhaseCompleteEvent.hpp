...
namespace PVZEvent
{
	...
	class PuzzlePhaseCompleteBonusEvent : public ThreeStateEventTemplate<0x429998, 8, 0x4299C2, 0x429AAE, MEM_ESP_ADD(0x34), REG_EDI, REG_ECX>
	{
	public:
		PuzzlePhaseCompleteBonusEvent(const char* str) : ThreeStateEventTemplate() { Init(str); };
		PuzzlePhaseCompleteBonusEvent(int address) : ThreeStateEventTemplate() { Init(address); };
		PuzzlePhaseCompleteBonusEvent() : ThreeStateEventTemplate() { Init("onPuzzlePhaseCompleteBonus"); };
	};
}